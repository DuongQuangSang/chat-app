...
export const sendFriendRequest = async (req, res) => {
  try {
    ...
    if (!userExits) {
      return res.status(404).json({ message: "Không thể gửi yêu cầu đến người dùng không tồn tại" });
    }
    ...
    const [alreadyFriend, exitingRequest] = await Promise.all([
    ...
    if (exitingRequest) {
      return res.status(400).json({ message: "Yêu cầu kết bạn đã được gửi và đang chờ đồng ý" });
    }
    ...
    console.error("Lỗi khi gọi addFriend", error);
...
export const getFriendRequests = async (req, res) => {
  try {
    ...
    const [sent, reviced] = await Promise.all([
      FriendRequest.find({ from: userId }).populate("to", populateFields),
      FriendRequest.find({ to: userId }).populate("from", populateFields),
    ]);

    return res.status(200).json({ sent, reviced });
  } catch (error) {
    console.error("Lỗi khi gọi getFriendsRequest", error);
    return res.status(500).json({ message: "Lỗi hệ thống" });
  }
};
